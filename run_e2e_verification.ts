...
async function main() {
  ...
  try {
    ...
    if (!pwdInput) throw new Error("Password input field not found");
    ...
    if (!generateBtn.asElement()) throw new Error("Generate PVC Card button not found");
    await (generateBtn.asElement() as any).click();
